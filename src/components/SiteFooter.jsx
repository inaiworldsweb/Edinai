...
const SiteFooter = () => {
  return (
    <footer className="site-footer" aria-label="Global footer">
      <div className="site-footer__sections" role="list">
        {productSections.map(({ title, items }) => (
          ...
          >
            <h3 id={`footer-${title.replace(/\s+/g, '-').toLowerCase()}`} className="site-footer__section-title">
              {title}
            </h3>
      ...
      <div className="site-footer__social" role="navigation" aria-label="Social media">
        {socialLinks.map(({ name, icon }) => (
          <button key={name} type="button" className="site-footer__social-link" aria-label={name}>
            {icon}
          </button>
        ))}
      </div>
